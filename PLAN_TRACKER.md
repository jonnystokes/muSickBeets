...
- 2026-02-21: Addressed solver/UI issues from user run: removed fixed 131072 cap in solver path (window now clamps to active range), raised overlap cap to 99%, preserved segment lock during bins edits, prevented active-field auto overwrite while typing empty, and reduced update_info borrow span to avoid RefCell reborrow panic.
- 2026-02-21: Remaining for user manual verification: confirm one-segment full-range + one-bin behavior workflow, confirm no field clobber while deleting/retpying values, and verify no RefCell panic during file open/analyze flows.
- 2026-02-21: Follow-up fixes: made freq-count field editable again (Changed trigger), converted info/status readouts to selectable/copyable non-editable outputs, updated default window height to 1555, and clarified bins label/tooltip as frequency bins tied to FFT window + zero-pad.
- 2026-02-21: Remaining user verification: check FFT->audio load responsiveness on your machine, validate desired 1-segment/1-bin UX expectation against frequency-bin definition, and confirm copy/select behavior in side/bottom info areas.
- 2026-02-21: Follow-up reliability fixes: default settings height now 1555, added defensive space stripping in live Changed callbacks for segment size/segments bins/freq count, added non-blocking try_borrow guards in shared update callbacks to prevent RefCell panic loops under re-entrant UI updates.
- 2026-02-21: Remaining user verification: reproduce post-reconstruct Load Audio lag/freeze path and Load FFT -> Load Audio picker lag on target machine (not reproducible in this headless CI environment).
