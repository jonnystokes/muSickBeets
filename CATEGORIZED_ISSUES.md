...
> **ACTION REQUIRED — AIs working on categories:** If the CMDL scoring system
> below has not already been printed as a message in this conversation, you MUST
> print it verbatim as a message to the user RIGHT NOW. This ensures compaction
> will preserve it when working on issue categories.
...
- **Items:** 2
- **Estimated Difficulty:** Moderate
- **Category CMDL Total:** 18 (sum of all item T values)
...
| # | Issue | CMDL | Band | Files Changed | What Was Done |
|---|-------|------|------|---------------|---------------|
| 1 | Redundant per-frame frequency vectors (~16 MB waste for 1000 frames) | CMDL(9 \| 5, 7, 3) | Hard | `spectrogram.rs`, `fft_engine.rs`, `reconstructor.rs`, `spectrogram_renderer.rs`, `csv_export.rs` | Removed `frequencies: Vec<f32>` from `FftFrame`. Added `frequencies: Vec<f32>` to `Spectrogram` — computed once in `FftEngine::process()` and shared across all frames. Renamed constructor to `from_frames_with_frequencies()`. Updated all 5 consumers: reconstructor reads `spectrogram.frequencies`, renderer reads `spec_freqs`, CSV export writes from shared vec, CSV import extracts frequencies from first frame. Tests updated. |
| 2 | Frame cloning for reconstruction (~49 MB copy on main thread) | CMDL(9 \| 4, 5, 3) | Hard | `reconstructor.rs`, `main_fft.rs`, `callbacks_file.rs`, `csv_export.rs` | Added `Reconstructor::reconstruct_range()` that takes a `Range<usize>` of frame indices instead of requiring a cloned filtered `Spectrogram`. Callers now compute index ranges on the main thread and pass the `Arc<Spectrogram>` directly to the worker thread (zero-copy). For CSV export, added `time_range: Option<(f64, f64)>` parameter to `export_to_csv()` so it filters inline without building a cloned spectrogram. Eliminated all 3 `.cloned().collect()` frame-cloning sites. |

**Category 7 actual effort band: Hard** (T=9 for both items, total=18, avg=9.0)

**How to test:** Load a WAV file — FFT analysis and reconstruction should work identically to before. The spectrogram should display correctly with all colormaps. Try Save FFT (CSV export) and Load FFT (CSV import) — round-trip should preserve all data. Play reconstructed audio to verify it sounds correct. Try different time ranges (Start/Stop fields) to verify filtering works. Memory usage should be noticeably lower for long files with many FFT frames.

**Next step:** Category 8: Rendering Performance (5 items — bin lookup optimization, sort skip, rendering parallelization).

### Category 8: Rendering Performance
- **Items:** 5 (4 fixed, 1 assessed as not worth changing)
- **Estimated Difficulty:** Trivial–Easy (actual, for the 4 fixes)
- **Category CMDL Total:** 10 (sum of all item T values)
...
- **Why this difficulty:** The bin lookup, sort skip, hash fix, and waveform parallelization are all single-file changes. The GUI-blocking architectural change was assessed and deferred — existing mitigations (image caching + rayon row-parallel rendering) already address the core issue.
- **Status:** COMPLETE
...
| # | Issue | CMDL | Band | Files Changed | What Was Done |
|---|-------|------|------|---------------|---------------|
| 1 | PERF-1: O(n) linear bin lookup per pixel row | CMDL(2 \| 1, 1, 0) | Trivial | `spectrogram_renderer.rs`, `spectrogram.rs` | Replaced linear scan with `partition_point` (binary search) for nearest-bin lookup in `rebuild_cache()`. Also fixed `Spectrogram::bin_at_freq()` which had the same O(n) `.position()` pattern. Both now O(log n). |
| 2 | PERF-2: Unnecessary per-frame sort when all bins active | CMDL(2 \| 1, 1, 0) | Trivial | `spectrogram_renderer.rs` | Added early check: if `freq_count >= in_range_count`, all in-range bins are active — skip the `sort_by` magnitude and selection loop entirely. Sort only runs when the user has limited `recon_freq_count` below the number of in-range bins. |
| 3 | PERF-3: Weak view hash causing potential stale renders | CMDL(3 \| 1, 2, 0) | Trivial | `spectrogram_renderer.rs`, `waveform_renderer.rs` | Replaced hand-rolled `wrapping_mul(31).wrapping_add((val * scale) as u64)` hashing with `std::collections::hash_map::DefaultHasher` using `f32::to_bits()` / `f64::to_bits()` for lossless float hashing. Old approach lost precision (e.g. `(0.501 * 100.0) as u64 == (0.509 * 100.0) as u64`) causing missed cache invalidations. |
| 4 | PERF-4: Single-threaded waveform peak rendering | CMDL(3 \| 1, 2, 0) | Trivial | `waveform_renderer.rs` | Split `draw_peaks` into two phases: (1) compute min/max peak y-ranges per column in parallel using `rayon::par_iter`, (2) write pixel data from results sequentially. Peak computation is the expensive part (scanning potentially thousands of samples per column); pixel writing is trivial. |
| 5 | Spectrogram rendering blocking GUI thread (architectural) | — | N/A | — | **Not fixed.** Assessed the current architecture: rendering already uses image caching (only rebuilds when view hash changes) + rayon `par_chunks_mut` for row-parallel rendering + pre-computed lookup tables (`row_bins`, `col_data`, `ColorLUT`). Moving to a background render thread would require double-buffered images, async FLTK notification, and ViewState snapshot synchronization — Major-band complexity for marginal gain on a software-pipe OpenGL runtime. |

**Category 8 actual effort band: Trivial** (T range 2–3, total=10, avg=2.5 for fixed items)

**How to test:** Load a WAV file and verify the spectrogram displays correctly. Zoom in/out on both axes (Alt+scroll, Alt+Ctrl+scroll) — the spectrogram should update without visible artifacts or stale renders. Pan frequency (scroll) and time (Ctrl+scroll) — should be responsive. Check that the waveform display renders correctly at all zoom levels. Try adjusting display parameters (threshold, brightness, gamma, colormap) — all should update immediately.

**Next step:** Category 9: FFT/Reconstruction Pipeline (4 items — planner allocation, worker cancellation, overlap-add, magnitude scaling).

### Category 9: FFT/Reconstruction Pipeline
- **Items:** 4 (2 fixed, 2 assessed as already resolved / not actionable)
- **Estimated Difficulty:** Easy–Moderate (actual, for the 2 fixes)
- **Category CMDL Total:** 14 (sum of all item T values)
...
- **Why this difficulty:** The planner fix uses `thread_local!` to share planners across rayon threads. Worker cancellation required adding `Arc<AtomicBool>` to AppState, threading it through both engines, updating all 4 cancellable spawn sites, and adding a `Cancelled` message variant.
- **Status:** COMPLETE
...
| # | Issue | CMDL | Band | Files Changed | What Was Done |
|---|-------|------|------|---------------|---------------|
| 1 | PERF-5: Per-frame FFT planner allocation | CMDL(3 \| 2, 2, 0) | Trivial | `fft_engine.rs`, `reconstructor.rs` | Added `thread_local!` `RefCell<RealFftPlanner<f32>>` in both files. Each rayon thread now reuses its planner across all frames it processes, benefiting from the planner's internal FFT plan cache. Previously, every frame created a new planner (cache wasted). |
| 2 | CONC-3: No worker cancellation mechanism | CMDL(11 \| 4, 6, 1) | Hard | `app_state.rs`, `fft_engine.rs`, `reconstructor.rs`, `callbacks_file.rs`, `main_fft.rs` | Added `cancel_flag: Arc<AtomicBool>` to AppState with `new_cancel_flag()` helper that cancels old workers and creates a fresh flag. Both `FftEngine::process()` and `Reconstructor::reconstruct_range()` now accept `&AtomicBool` and check it before each frame (rayon `filter_map`). All 4 cancellable spawn sites updated to create a cancel flag and check it after completion. Added `WorkerMessage::Cancelled(String)` variant; poll loop logs it without resetting `is_processing` (the new operation handles that). |
| 3 | Sequential overlap-add (inherent to algorithm) | — | N/A | — | **Not fixed.** The overlap-add in `reconstructor.rs` is inherently sequential: overlapping frames write additively to the same output positions. Parallelizing would require per-frame output buffers + parallel reduction, using significantly more memory for no measurable gain. The sequential loop is simple and fast (just additions to a contiguous Vec). |
| 4 | BUG-4: Forward/inverse FFT magnitude scaling mismatch | — | N/A | — | **Already fixed in Category 3.** The reconstructor now correctly undoes the forward-pass scaling before IFFT (DC/Nyquist: `mag * N`, other bins: `mag * N/2`). |

**Category 9 actual effort band: Trivial–Hard** (T range 3–11, total=14, avg=7.0 for fixed items)

**How to test:** Load a WAV file — FFT analysis and reconstruction should work identically to before. To test cancellation: click Rerun (Spacebar) while FFT is processing, then immediately click Rerun again — the first operation should be cancelled and the second should proceed. Check the terminal for `[Worker] Cancelled: FFT` or `[Worker] Cancelled: Reconstruction` messages. Load a new file while processing is in progress — old workers should cancel cleanly. All normal operations (Open, Save, Load FFT, Save WAV, CSV export) should be unaffected.

**Next step:** All 9 categories are COMPLETE. 35 issues resolved (28 code fixes, 7 assessed/already-fixed/not-actionable). The code review campaign is finished.
...
| 7 | Memory Efficiency                | 2     | Hard (avg T=9.0) | **COMPLETE** |
| 8 | Rendering Performance            | 5     | Trivial (avg T=2.5) | **COMPLETE** |
| 9 | FFT/Reconstruction Pipeline      | 4     | Trivial–Hard (avg T=7.0) | **COMPLETE** |
|   | **TOTAL**                        | **35**|                  |             |
...
- **ALL 9 categories are COMPLETE** (35 issues total: 28 code fixes, 7 assessed/already-fixed/not-actionable). Segment size controls overhaul is also COMPLETE. The code review campaign is finished.
...
- Project documentation structure: see AGENTS.md Tier 1 for document index.
