...
pub fn solve(input: SolverInput) -> SolverOutput {
    ...
    match input.last_edited {
        ...
        LastEditedField::BinsPerSegment => {
            if let Some(target_segments) = input.target_segments_per_active {
            ...
            } else if let Some(target_bins) = input.target_bins_per_segment {
...
fn solve_window_for_segments(
    ...
    let overlap_ratio = overlap_percent.clamp(
        constraints.min_overlap_percent,
        constraints.max_overlap_percent,
    ) / 100.0;
    let hop_factor = (1.0 - overlap_ratio).max(0.001);
...
mod tests {
    ...
    #[test]
    fn bins_edit_keeps_single_segment_lock_when_present() {
        let out = solve(SolverInput {
        ...
        assert_eq!(out.segments_per_active, 1);
        assert_eq!(out.window_length, 88_200);
        assert_eq!(out.bins_per_segment, 44_101);
    }
    ...
    fn solver_enforces_hard_constraints() {
        ...
        assert_eq!(out.window_length, 1_000_000);
        assert!(out.window_length % 2 == 0);
        assert_eq!(out.overlap_percent, 99.0);
        assert_eq!(out.bins_per_segment, 500_001);
    }
